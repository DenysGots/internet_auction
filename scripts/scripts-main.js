...
/* 	localStorage.removeItem("users"); */
	...
	function createNewLot() {
		...
		if (!newLotCharacteristics.every(function (obj) {
 		...
 		initialContent.unshift({
			...
			material: material,
			data_country_of_origin: data_origin,
	...
	function createLotsPages() {
		...
		for (var i = 0; i < initialContent.length; i += 1) {
			...
			lotPage.getElementsByClassName("lot-page_country-of-origin")[0].innerHTML = initialContent[i].country_of_origin;
	...
	function createLotsPreviews() {
		...
		function addStandartLotAttributes(elem) {
			...
			elem.setAttribute("data-country-of-origin", initialContent[i].data_country_of_origin);
			elem.setAttribute("data-year-of-manufacture", initialContent[i].data_year_of_manufacture);
			elem.setAttribute("data-country-of-origin-value", initialContent[i].country_of_origin);
	...
	function createAsideFlitersList() {
			...
			lotCharacteristicsFilters = [
				{
					...
					data_type_characteristic: "country-of-origin",
		...
		Array.prototype.slice.call(lotsPreviews).forEach(function (obj) {
			if (obj.getAttribute("data-current-selected") === "current-selected") {
				if (obj.hasAttribute("data-country-of-origin") && obj.getAttribute("data-country-of-origin") !== "undefined") {
					lotCharacteristicsFilters[0].characteristicFilters.push({
						value: obj.getAttribute("data-country-of-origin-value"),
						data_value: obj.getAttribute("data-country-of-origin")
					});
				};
					...
					Array.prototype.slice.call(lotsPreviews).forEach(function (obj) {
						if (obj.getAttribute("data-current-selected") === "current-selected") {
							...
							switch(e.target.getAttribute("data-type")) {
								case "country-of-origin":
									if (
										obj.getAttribute("data-country-of-origin") === e.target.getAttribute("id")
										&&
										!e.target.checked
									) {
										hideElements(obj);
